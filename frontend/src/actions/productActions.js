...
export const getProduct = (keyword="",currentPage=1,price=[0,100000],category,rating=0) => async (dispatch) => {
    try {
      ...
      let link = `/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gt]=${price[0]}&price[lt]=${price[1]}`
      // if(price){
      //   link = `/api/v1/products?price[lte]=${price[0]}&price[gte]=${price[1]}`
      //  }else 
       if(rating){
       link = `/api/v1/products?keyword=${keyword}&page=${currentPage}&rating=${rating}`;
      }else if (category) {
...
export const createProduct = (productData) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.post(
      `/api/v1/admin/products/new-product`,
      productData,
      config
    );  
